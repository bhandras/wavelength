	...
	*commitResult, error) {
	...
	for _, out := range request.Outputs {
		...
		for inputIdx := range remaining {
			...
			committedInput := result.inputs[inputIdx]
			result.outputs = append(result.outputs, commitOutput{
				...
				previousAnchorOutpoint: committedInput.
					anchorOutpoint,
			})
		}
		if amount != 0 {
...
// TestBatchRootInputMerge checks a path for two inputs of one issuance.
func TestBatchRootInputMerge(t *testing.T) {
	t.Parallel()

	issuanceID := tapsdk.AssetID{0x01}
	assetRef := tapsdk.AssetRefFromAssetID(issuanceID)
	...
	output := commitOutput{
		...
		anchorValueSat: 330,
		proofBlob:      []byte("transition"),
		opTrueWitness: [][]byte{
			{
				0x51,
			},
		},
		previousAnchorOutpoint: secondOutpoint,
	}
	req := &BatchAnchorRequest{
		Sources: []BatchAnchorSource{
			{
	...
	rootInput, err := batchRootInput(
		req, output, inputs, wire.OutPoint{Index: 3}, []byte("anchor"),
		func(value *tapsdk.AssetProofPath) ([]byte, error) {
			path = value.Clone()
	...
	require.Equal(t, []byte("second-proof"), path.ConfirmedBaseProof)
	require.Len(t, path.Steps[0].CoInputPaths, 1)
	require.Equal(
		t, []byte("first-proof"),
		path.Steps[0].CoInputPaths[0].ConfirmedBaseProof,
	)

	verifier, ok := rootInput.Verifier.(*treePathVerifier)
	require.True(t, ok)
	require.Equal(t, []tapsdk.Outpoint{
		secondOutpoint, firstOutpoint,
	}, verifier.steps[0].previousOutpoints)
}
...
// TestBatchAnchorJournal checks replay and unknown commit outcomes.
