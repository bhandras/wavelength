...
func batchRootInput(req *BatchAnchorRequest, output commitOutput,
	...
	previousOutpoint := output.previousAnchorOutpoint
	if previousOutpoint == (tapsdk.Outpoint{}) {
	...
	if base.anchorOutpoint != previousOutpoint {
		return TreeRootAssetInput{}, fmt.Errorf("transition base " +
			"does not match packet input zero")
	}
